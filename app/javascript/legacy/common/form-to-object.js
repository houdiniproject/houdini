...
module.exports = function(form) {
	...
	const result = {}
	toArr(form.querySelectorAll('input, textarea, select'))
  .filter(function(n) { return n.hasAttribute('name') })
  .map(function(n) {
    const keys = n.getAttribute('name').split('.')
...
function deepSet(keys, val, obj, options) {
	const exceptLast = keys.slice(0, keys.length-1)
	const last = keys[keys.length-1]
	const nested = exceptLast.reduce(function(nestedObj, key) {
