...
module InsertDuplicate
  def self.campaign(campaign_id, profile_id, new_nonprofit = nil)
    ...
    Qx.transaction do
      ...
      begin
        dupe.update_attribute(:main_image, campaign.main_image) unless !campaign.main_image
      rescue
        Aws::S3::Errors::NoSuchKey
      end

      begin
        dupe.update_attribute(:background_image, campaign.background_image) unless !campaign.background_image
      rescue
        Aws::S3::Errors::NoSuchKey
      end
  ...
  def self.event(event_id, profile_id, new_nonprofit = nil)
    ...
    Qx.transaction do
      ...
      begin
        dupe.update_attribute(:main_image, event.main_image) unless !event.main_image
      rescue
        Aws::S3::Errors::NoSuchKey
      end

      begin
        dupe.update_attribute(:background_image, event.background_image) unless !event.background_image
      rescue
        Aws::S3::Errors::NoSuchKey
      end
