...
module QueryPayments
  ...
  def self.full_search_expr(npo_id, query)
    ...
    if %w[asc desc].include? query[:sort_amount]
      expr = expr.order_by("payments.gross_amount #{query[:sort_amount]}")
    end
    if %w[asc desc].include? query[:sort_date]
    ...
    if %w[asc desc].include? query[:sort_type]
      expr = expr.order_by("payments.kind #{query[:sort_type]}")
    end
    if %w[asc desc].include? query[:sort_towards]
  ...
  def self.select_to_filter_search(npo_id, query)
    ...
    expr = SearchVector.query(query[:search], expr) if query[:search].present?
    if %w[asc desc].include? query[:sort_amount]
      expr = expr.order_by("payments.gross_amount #{query[:sort_amount]}")
    end
    if %w[asc desc].include? query[:sort_date]
  ...
  def self.for_payout(npo_id, payout_id)
    ...
    Qx.select(QuerySupporters.supporter_export_selections)
    Qx.select(
  ...
  def self.find_payments_where_too_far_from_charge_date(id = nil)
    ...
    pay = pay.where("id = ?", id) if id
    pay = pay.where.not(date: nil).order("id ASC")
    pay.all.each do |p|
      next unless p.offsite_payment.nil?
