...
class Supporter < ApplicationRecord
  ...
  def cleanup_address
    if address.present? && address_line2.present?
      assign_attributes(address_line2: nil, address: address + " " + address_line2)
    end
  ...
  def update_primary_address
    if changes.slice(*ADDRESS_FIELDS).any? # changed an address field
      if filled_address_fields?
        if primary_address.nil?
          addresses.build(address_field_attributes)
        else
          primary_address.update(address_field_attributes)
        end
      elsif primary_address.present?
        prim_addr = primary_address
        update(primary_address: nil)
        addresses.delete(prim_addr)
        prim_addr.destroy
      end
    end
  end
