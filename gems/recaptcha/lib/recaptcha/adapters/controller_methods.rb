  ...
  module Adapters
    module ControllerMethods
      ...
      def check_recaptcha(options = {})
        return {success:true} if Recaptcha.skip_env?(options[:env])

        recaptcha_response = options[:response] || recaptcha_response_token(options[:action])

        begin
          verified = if Recaptcha.invalid_response?(recaptcha_response)
            {success: false, error: RecaptchaError.new, error_message: "Invalid Response"}
          else
            unless options[:skip_remote_ip]
              remoteip = (request.respond_to?(:remote_ip) && request.remote_ip) || (env && env['REMOTE_ADDR'])
              options = options.merge(remote_ip: remoteip.to_s) if remoteip
            ...
            Recaptcha.check_via_api_call(recaptcha_response, options)
          end
          ...
          if verified && verified[:success]
            verified
             ...
             verified[:error] = VerifyError.new
             verified[:error_message] = Recaptcha::Helpers.to_error_message(:verification_failed)
