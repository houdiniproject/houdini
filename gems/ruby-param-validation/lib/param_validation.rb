...
class ParamValidation
  ...
  @@validators = {
    ...
    not_included_in: lambda { |val, arg, data|
      begin
        !arg.include?(val)
      rescue
        false
    ...
    included_in: lambda { |val, arg, data|
      begin
        arg.include?(val)
      rescue
        false
    ...
    min_length: lambda { |val, arg, data|
      begin
        val.length >= arg
      rescue
        false
    ...
    max_length: lambda { |val, arg, data|
      begin
        val.length <= arg
      rescue
        false
    ...
    length_range: lambda { |val, arg, data|
      begin
    ...
    is_a: lambda { |val, arg, data| arg.is_a?(Enumerable) ? arg.any? { |i| val.is_a?(i) } : val.is_a?(arg) },
  ...
  @@messages = {
    ...
    is_a: lambda { |h| "#{h[:key]} should be of the type(s): #{h[:arg].is_a?(Enumerable) ? h[:arg].join(", ") : h[:arg]}" },
