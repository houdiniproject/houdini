...
class CopyNamingAlgorithm
  ...
  def create_copy_name(name_to_copy)
    ...
    base_name = name_to_copy.gsub(/#{Regexp.quote(self.copy_addition)}(#{Regexp.quote(separator_before_copy_number)}\d+)?\z/,'')
    name_entities_to_check_against = get_already_used_name_entities(base_name).collect{|entity| self.get_name_for_entity(entity)}.to_a
  ...
  def generate_name(name_to_copy, copy_num)
    what_to_add = self.copy_addition + self.separator_before_copy_number + generate_copy_number(copy_num)
