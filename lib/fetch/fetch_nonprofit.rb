...
module FetchNonprofit
	...
	def self.with_params(params, administered_nonprofit=nil)
		if params[:state_code] && params[:city] && params[:name]
			return Nonprofit.find_via_cached_key_for_location(params[:state_code], params[:city], params[:name])
		elsif params[:nonprofit_id] || params[:id]
			return Nonprofit.find_via_cached_id(params[:nonprofit_id] || params[:id])
