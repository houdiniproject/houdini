...
module MaintainDonationValidity
  ...
  def self.change_all_donation_to_supporter(d, new_supporter)
    ...
    d.activities&.each{|i| i.supporter = new_supporter; i.save!}
    d.payments&.each{|i| i.supporter = new_supporter; i.save!}

    d.charges&.each{|i| i.supporter = new_supporter; i.save!}
    if (d.card)
