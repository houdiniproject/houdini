    ...
    context 'main testing' do
      ...
      it 'id, updated_at, created_at changes are stripped' do
        ...
        Timecop.freeze(2020, 9, 1, 12, 0, 0) {
          ...
          expected = {custom_field_master_id: 25, value: "CFM value 25", created_at: Time.now, updated_at: Time.now, supporter_id: @supporters[:np_supporter_with_add][:entity].id}.to_deprecated_h.with_indifferent_access
          ...
          expect(result_tag.attributes.to_deprecated_h.with_indifferent_access.reject {|k, _| k == 'id'}).to eq(expected)
