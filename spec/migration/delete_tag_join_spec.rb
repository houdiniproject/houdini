  ...
  it 'copying should be safe and reverable' do
    ...
    3.times do |j|
      ...
      5.times do |i|
        ...
        last_times_for_supporter_ids[i] = time if j == 3
    ...
    tag_join_groups = TagJoin.all.to_a.group_by { |tj| "#{tj.supporter_id}_#{tj.tag_master_id}" }.select { |_k, v| v.count > 1 }.to_a
    ...
    expect(Qx.select('*').from(:tag_joins_backup).where('id IN ($ids)', ids: ids_to_delete).execute).to match_array(our_test_tj.map(&:attributes))
