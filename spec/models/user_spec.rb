...
RSpec.describe User, type: :model do
  ...
  describe ".send_reset_password_instructions" do
    context "with a valid email address" do
      context "when the user hasn't requested a reset password token before" do
        it "returns a User" do
          ...
          expect(User.send_reset_password_instructions({email: "evil_hacker@netflix.io"})).to be_a User
        end

        it "returns the correct user" do
          user = create(:user, email: "sneaky_scammer@fb.net")
          expect(User.send_reset_password_instructions({email: "sneaky_scammer@fb.net"})).to eq(user)
        end

        it "returns an object with no errors" do
          create(:user, email: "gone_phishing@twtr.gov")
          expect(User.send_reset_password_instructions({email: "gone_phishing@twtr.gov"}).errors.messages).to eq({})
        end
      end
      ...
      context "when a user has requested a reset password token recently (< 5 min ago)" do
        it "returns the correct user" do
          ...
          User.send_reset_password_instructions({email: "fraud_fool@insta.org"})
          expect(User.send_reset_password_instructions({email: "fraud_fool@insta.org"})).to eq(user)
        end
        ...
        it 'adds "can\'t reset password because a request was just sent" error to returned user' do
          ...
          User.send_reset_password_instructions({email: "perilous_programmer@snap.com"})
          expect(
            User.send_reset_password_instructions({email: "perilous_programmer@snap.com"}).errors.messages[:base]
    ...
    context "with an invalid email address" do
      it "returns a User" do
        expect(
          User.send_reset_password_instructions({email: "unknown_goose@domain.net"})
        ).to be_a User
      end

      it "returns a blank User" do
        expect(
          User.send_reset_password_instructions({email: "unknown_panda@domain.net"}).id
        ).to be_nil
      end
